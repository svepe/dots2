...
- **Verify with `scripts/90-kde-shortcuts.sh --check`** — reads every binding back out of the daemon and exits non-zero on any that isn't active, writing and registering nothing. Run it after a reboot to confirm what the config alone produced; that is the check whose absence let a fresh host come up with stock defaults.
